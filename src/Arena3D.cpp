...
Vector3d<double> Arena3D::GetEntranceCenterFromPosition(const Vector3d<double> &point) {
    Entrance entrance = *(GetWallFromPosition(point)->GetOpenEntrance());
    return entrance.GetCenter();

}

Vector3d<double> Arena3D::GetEntranceCenterFromPosition(const double &x, const double &y, const double&z) {
    return this->GetEntranceCenterFromPosition(Vector3d<double>(x, y, z));

}

Wall *Arena3D::GetWallFromPosition(const Vector3d<double> &point) {
    int current_room = GetRoomFromPosition(point);
    int next_room = (current_room + 1) % walls.size();
    return &(walls[next_room]);

}

Wall *Arena3D::GetWallFromPosition(const double &x, const double &y, const double &z) {
    return this->GetWallFromPosition(Vector3d<double>(x, y, z));
}
